...
from redkg.visualization.equations.calc_edge_line_width import (
    calculate_edge_line_width
)
from redkg.visualization.equations.calc_font_size import calculate_font_size
from redkg.visualization.equations.calc_vertex_line_width import (
    calculate_vertex_line_width
)
from redkg.visualization.equations.calc_vertex_size import (
...
class SizeConstructor:
    ...
    def __call__(self, contract: SizeConstructorContract) -> tuple:
        ...
        v_line_width = fill_sizes(
            contract.vertex_line_width,
            _vertex_line_width,
            contract.vertex_num
        )

        e_line_width = fill_sizes(
            contract.edge_line_width,
            _edge_line_width,
            edge_list_length
        )

        font_size = _font_size if contract.font_size is None \
            else contract.font_size * _font_size
