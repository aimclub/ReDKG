...
class CorePhysicalModel:
    ...
    def __init__(self, contract: CoreModelContract):
        ...
        self.node_attraction = contract.forces.get(
            self.__node_attraction_key,
            None
        )
        self.node_repulsion = contract.forces.get(
            self.__node_repulsion_key,
            None
        )
        self.edge_repulsion = contract.forces.get(
            self.__edge_repulsion_key,
            None
        )
        self.center_gravity = contract.forces.get(
            self.__center_of_gravity_key,
            None
        )
    ...
    def build(
        ...
        for it in range(max_iter):
            position, velocity, stop_iterations = self.__make_one_step(
                position,
                velocity,
                H,
                epsilon,
    ...
    def __make_one_step(self, position, velocity, H, epsilon, damping, delta):
        ...
        vertex_to_edge_distance = euclidean_distances(
            position,
            edge_center
        ) * H
        ...
        if self.node_attraction is not None:
            force_modifier = (
                self.__node_attraction(
                    position,
                    edge_center,
                    vertex_to_edge_distance
                ) * self.node_attraction
            )
            force += force_modifier

        if self.node_repulsion is not None:
            force_modifier = self.__node_repulsion(
                position,
                vertex_to_vertex_distance
            )
        ...
        if self.center_gravity is not None:
            ...
            for center, gravity, mask in zip(
                    self.centers,
                    self.center_gravity,
                    masks
            ):
                v2c_dist = euclidean_distances(
                    position,
                    center.reshape(1, -1)
                ).reshape(-1, 1)
                force_modifier = self.__center_gravity(
                    position,
                    center,
                    v2c_dist
                ) * gravity * mask
    ...
    def __node_repulsion(position, vertex_to_vertex_distance, k=1.0):
        ...
        force_direction = (position[:, np.newaxis, :]
                           - position[np.newaxis, :, :])
        force_direction_length = np.linalg.norm(force_direction, axis=2)
    ...
    def __edge_repulsion(edge_center, H, edge_to_edge_dist, k=1.0):
        ...
        force_scale = k / (distance**2)
        force_direction = (edge_center[:, np.newaxis, :]
                           - edge_center[np.newaxis, :, :])
        force_direction_length = np.linalg.norm(force_direction, axis=2)
        force_direction_length[r, c] = np.inf
        force_direction = safe_div(
            force_direction,
    ...
    def __center_gravity(position, center, vertex_to_vertex_distance, k=1):
        ...
        force_direction = (center[np.newaxis, np.newaxis, :]
                           - position[:, np.newaxis, :])
        force_direction_length = np.linalg.norm(force_direction, axis=2)
        force_direction = safe_div(
            force_direction,
