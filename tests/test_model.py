...
def test_model():
    ...
    neg_tail_triple = torch.stack( (head.repeat(neg_tail.size()), 
                                    relation.repeat(neg_tail.size()), 
                                    neg_tail), dim=1)
    neg_head_triple = torch.stack( (neg_head,
                                    relation.repeat(neg_head.size()), 
                                    tail.repeat(neg_head.size()) ), dim=1)
    ...
    negative_head_score = scores[pos_triple.shape[0]+neg_tail_triple.shape[0]:pos_triple.shape[0]+neg_tail_triple.shape[0]+neg_head_triple.shape[0]]
